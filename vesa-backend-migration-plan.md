...
**Decided — implemented.** Ships as `@vesa-gg/core`, an installable TypeScript package rather than a third always-on service, distributed via git-tag dependency. `package.json`'s `prepare` script runs the build on `npm install` (git-tag installs don't ship a pre-built `dist/`), and the tag/release GitHub Actions mirror scrim-bot's existing `bump-version.sh` machinery. Both consumers pin to an exact tag, not a range, and bump deliberately — this is shared code two production surfaces depend on, so silent floating upgrades are the wrong default.

**Constraint this puts on the package — not yet fully met:** since it needs to run inside VESAWeb's Cloudflare Worker as well as Node, it must be written against **web-standard APIs only** — `fetch`, Web Crypto, `URL`, no `fs`, no Node-only SDKs (`@googleapis/sheets`, `@huggingface/hub` stay in the apps that use them, not in the shared package). This isn't a hypothetical restriction — VESAWeb already hit it once and worked around it by hand-rolling a Web Crypto JWT signer instead of using `googleapis`. See "Divergences and concerns since implementation started" at the end of this document for the specific files that currently violate it.

---

## 2. Handling the two authorization modes

**Decided — partially implemented.** The design below — every backend action takes an `Actor` argument, with an admin case and a user case — is the settled model, not an open proposal. Built so far: `AuthService.memberIsAdmin` takes a plain `roleIds: string[]` instead of a `GuildMember`, and an `Actor` type (identity + `roleIds`) threads through `RosterService`/`SignupService` in place of separately-passed `(user, roleIds)` pairs — that's the `user` case below, built early because scrim-bot's own commands needed it too. Not yet built: the `{kind: "admin"}` case (still needed, for a caller with no real Discord identity behind it, e.g. an internal scheduled job), JWT verification against Nhost's JWKS, and the Discord REST role-lookup path for VESAWeb's Worker.

Every backend action (the equivalent of today's service methods — `closeScrim`, `addPrio`, `insertLeagueSignup`, etc.) takes an actor argument that's one of:
...
**Decided — not yet implemented.** Planned to happen after step 5 (scrim-bot wired to consume `core`), not before it — reversing the original de-risk-first sequencing this section used to recommend. The approach has also changed from what this plan originally proposed: rather than moving scrim-bot's tracked `nhost/nhost/` project into this repo and reconciling it by hand against production, the decision is to initialize a fresh Nhost CLI project inside `core`, connect it to the production project, and let the CLI pull the live schema down directly.

That doesn't remove the underlying risk: production has never actually been driven by what's in git, and the tracked migrations in scrim-bot are a *reconstruction*, not a true history — there's no guarantee they'd produce an identical schema if replayed from scratch (manual tweaks made directly in the Hasura console wouldn't be captured). Confirming the freshly-pulled schema actually matches what's live is still a necessary step before turning on automatic deploys, whichever way it's pulled down.

**Automatic deploys, once the schema is pulled down:**

- **Decided:** use Nhost's built-in Git integration (connect the Nhost project to `core` + branch in the Nhost dashboard), not a hand-rolled GitHub Action — it's built for exactly this, and the subdirectory the project lives in is configurable, so nesting it under `/nhost` next to the package source is fine.
- Mirror VESAWeb's existing promotion ritual rather than inventing a new one: point the **dev** Nhost environment at `core`'s `main` (auto-deploy on every merge, matching `dev.vesa.gg`), and **prod** at a `release` branch promoted via `git merge --ff-only main` (matching how `vesa.gg` is promoted today). One mental model for both app code and schema changes.
...
**Decided — implemented.** `core`, published as `@vesa-gg/core` — signals "shared domain library" rather than a hosted, independently-running backend, matching §1.

---

## 5. Should the backend be DB-provider agnostic?

**Decided — implemented.** Kept the `DB` abstract-class-plus-mock pattern (`DB`/`NhostDb`/`DbMock`, moved into `core` as-is) rather than pursuing full multi-provider portability. Nhost's Auth, Storage, and migration/metadata tooling are already Postgres+Hasura-specific by construction, so a swappable `DB` interface would only ever abstract the query layer — "provider agnostic" would be more illusion than insurance. The existing abstraction earns its keep for a narrower, real reason instead: `DbMock` makes services unit-testable without a live database.

Two follow-on items from this decision:

- **Deferred, not a current priority.** Splitting the monolithic `DB` class into domain-scoped repositories (`ScrimRepository`, `PlayerRepository`, `LeagueRepository`, …), as ADR-0007 and CLAUDE.md suggest. Only `League` ended up with its own repository split so far, driven by the Sheets integration rather than this general pattern.
- **Open, not yet fixed.** The GraphQL-injection issue ADR-0010/CLAUDE.md flag: `db.ts` still builds several queries via unescaped string interpolation (e.g. `insertPlayerIfNotExists` does `discord_id: "${discordId}"` directly) — low cost to fix now, much higher cost once two production surfaces depend on the shared query builder instead of one.

---

## Suggested phased sequence

1. **Reconcile production schema** (§3) — a fresh Nhost CLI project connected to production, pulling the live schema down directly. **Status: not started.** No longer planned first — see §3 for the revised sequencing (now after step 5) and approach.
2. **Create the new repo** (`core`, per §4); move the framework-agnostic pieces out of scrim-bot — `src/db`, `src/services`, `src/models`. Leave Discord-specific glue (`Client.ts`, commands, events, the `discord.js` dependency itself) in scrim-bot. **Status: done**, except the `nhost/` project — that's deferred along with step 1, see §3.
